 ...
 **In our setting**: endTB and TB-PRACTECAL use heterogeneous standards of care ($c_1 \neq c_2$), so $\phi \neq 0$ and the traditional estimator generally targets $\theta = \psi - ## \phi$ rather than $\psi$. **Bias is systematic.**
:::
...
$$
\Delta := \mathbb{E}[Y(a)-Y(b)\mid\Omega_{\text{trt}}] - \mathbb{E}[Y(a)-Y(b)\mid\Omega_{\text{untrt}}] = \int \tau_{ab}(L)\{f(L\mid\Omega_{\text{trt}}) - f(L\mid\Omega_{\text{untrt}})\}\,dL.
$$
Violation arises from **interaction between treatment effect heterogeneity and covariate imbalance** across subpopulations. In-practice, within-trial randomization makes $f(L\mid\Omega_{\text{trt}}) \approx f(L\mid\Omega_{\text{untrt}})$, partially mitigating this.
:::
...
| Method                      | Uses randomization | Uses all arms data| Statistically efficient | More Assumption | 
| --------------------------- | :----------------: | :-----------: | :-----------: | :---------------------: | 
